...
bool Coordinator::initRepair(string method, string scenario, int failnodeid) {
  ...
  if (_codename == "Clay") {
    ...
    _ec = new RSCONV(_ecn, _eck, _ecw, param);
  } else if (_codename == "RSPIPE") {
    _ec = new RSPIPE(_ecn, _eck, _ecw, param);
  } else {
  ...
  if (method == "centralize") {
    ...
    _sol = new ParallelSolution(batchsize, standbysize, agentnum, 0);
    _sol->init(_ss->getStripeList(), _ec, _codename, _conf);
  }
...
bool Coordinator::initRepair(string method, string scenario,
                             vector<int> failnodeids) {

  _method = method;
  _scenario = scenario;
  _failnodeids = failnodeids;
  vector<string> param;
  // 1. create ECBase
  ...
  // 3. fail2repair
  if (scenario == "standby") {
    for (auto it : _failnodeids) {
      _fail2repair.insert(make_pair(it, _conf->_agentsIPs.size()));
    }
  }
...
vector<RepairBatch *> Coordinator::genRepairSolution() {
  ...
  _sol->genRepairBatches(1, _failnodeids, _conf->_agentsIPs.size(), _scenario,
                         false, _conf->_batchmethod);
...
int Coordinator::genRepairSolutionAsync() {
  ...
  _sol->genRepairBatches(_failnodeids.size(), _failnodeids,
                         _conf->_agentsIPs.size() + _conf->_repairIPs.size(),
                         _scenario, true, _conf->_batchmethod);
  return 0;
}

void Coordinator::repair() {
  cout << "Coordinator::repair start" << endl;
  ...
  while (_sol->hasNext()) {
    ...
    cout << "Coordinator:: repair  genRepairTasks start" << endl;
    ...
    cout << "Coordinator::repair batch" << curbatch->getBatchId()
         << " get batch time = " << t1 << ", repair time = " << t2
         << ", stripenum = " << curbatch->getStripeList().size() << endl;
  }
  ...
  for (int i = 0; i < batch_list.size(); i++) {
    ...
    cout << "avg time = "
         << repair_batch_vec[i] / batch_list[i]->getStripeList().size() << endl;
    delete batch_list[i];
  }
