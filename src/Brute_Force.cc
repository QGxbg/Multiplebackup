...
void simple_search(Stripe* currStripe,vector<int>& curres, int curidx,vector<int> itm_idx,vector<int> candidates,double* found,double total,
    ...
    if(curidx ==itm_idx.size()){
        ...
        //     cout<<"load_now < min_load"<<load_now <<"<"<< *min_load<<endl;
        //     if(bdwt_now <  *min_bdwt)
        //        *min_load = load_now;
        //        //currStripe->_load = *min_load;

        //         *min_bdwt = bdwt_now;
        //         //currStripe -> _bdwt = *min_bdwt;
        // }

       

        // if(currStripe->getLoad() > load_now  && load_now != 0){
        //     currStripe->_load = load_now;
        //     if(currStripe->getBdwt() > bdwt_now){
        //         currStripe->_bdwt = bdwt_now;
        //     }
        // }
        // cout<<"bdwt:"<<currStripe->getBdwt()<<endl;
        // cout<<"load:"<<currStripe->getLoad()<<endl;
...
// The size of the solution space
  ...
  int min_bdwt_mlp = -1;
  for (auto item: max2bwlist[min_bdwt_mlp]) {
      if (min_bdwt_mlp == -1)
          min_bdwt_mlp = item;
