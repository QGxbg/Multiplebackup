...
void CentSolution::genRepairBatches(int num_failures,
  ...
  if (num_failures == 1) {
    ...
    genRepairBatchesForMultipleFailure(fail_node_list, num_agents, scenario,
                                       enqueue);
  }
...
void CentSolution::genRepairBatchesForSingleFailure(int fail_node_id,
                                                    int num_agents,
                                                    string scenario,
                                                    bool enqueue) {

  // cout << "CentSolution::genRepairBatchesForSingleFailure.fail_node_id = " <<
  // fail_node_id << endl;

  // 0. we first figure out stripes that stores a block in $fail_node_id
  filterFailedStripes({fail_node_id});
  // cout << "CentSolution::genRepairBatchesForSingleFailure.stripes to repair:
  ...
  // cout << "CentSolution::genRepairBatchesForSingleFailure.num batches = " <<
  // _num_batches << endl;
  ...
  for (int batchid = 0; batchid < _num_batches; batchid++) {
    ...
    for (int i = 0; i < _batch_size; i++) {
      ...
      if (stripeidx < _stripes_to_repair.size()) {
        ...
        genCentralizedColoringForSingleFailure(
...
void CentSolution::genRepairBatchesForMultipleFailure(vector<int> fail_node_ids,
                                                      int num_agents,
                                                      string scenario,
                                                      bool enqueue) {
  ...
  LOG << "CentSolution::genRepairBatchesForMultipleFailure.num batches = "
      << _num_batches << endl;
  ...
  for (int batchid = 0; batchid < _num_batches; batchid++) {
    ...
    for (int i = 0; i < _batch_size; i++) {
      ...
      if (stripeidx < _stripes_to_repair.size()) {
        ...
        // 1.2 generate centralized coloring for the current stripe
        unordered_map<int, int> curcoloring;
        genCentralizedColoringForMutipleFailure(
            curstripe, curcoloring, fail_node_ids, num_agents, scenario);
        // 1.3 set the coloring result in curstripe
        curstripe->setColoring(curcoloring);
        // 1.4 evaluate the coloring solution
        curstripe->evaluateColoring();
...
void CentSolution::genCentralizedColoringForSingleFailure(
  ...
  for (int i = 0; i < leaves.size(); i++) {
    ...
    // cout << "dagidx: " << dagidx << ", blkidx: " << blkidx << ", nodeid: " <<
    // nodeid << endl;
...
void CentSolution::genCentralizedColoringForMutipleFailure(
  ...
  // 2.1 avoid fail nodeid
  avoid_node_ids.insert(avoid_node_ids.end(), fail_node_ids.begin(),
                        fail_node_ids.end());
