...
ParallelSolution::ParallelSolution(int batchsize, int standbysize,
                                   int agentsnum, int method) {
  // cout << "ParallelSolution::constructor. batchsize = " << batchsize << ",
  // standbysize = " << standbysize << ", agentsnum = " << agentsnum << ",
  // method = " << method << endl;
...
void ParallelSolution::ParallelSolution_standby(int batchsize, int standbysize,
                                                int agentsnum) {
  _batch_size = batchsize;
  _standby_size = standbysize;
  _agents_num = agentsnum;
  // LOG << "ParallelSolution::constructor. _batch_size = " << _batch_size <<
  // endl;
}
...
void ParallelSolution::genRepairBatches(int num_failures,
  ...
  int eck = _ec->_k;
  int ecw = _ec->_w;
  // string offline_solution_path =
  // _conf->_tpDir+"/"+_codename+"_"+to_string(ecn)+"_"+to_string(eck)+"_"+to_string(ecw)+".xml";
  // LOG << "offline solution path: " << offline_solution_path << endl;
  // _tp = new TradeoffPoints(offline_solution_path);

  // Xiaolu comment
  // enqueue == true: generated RepairBatch is pushed into a queue in
  // SolutionBase::_batch_queue enqueue == false: generated RepairBatch is added
  // into a vector in SolutionBase::_batch_list
  _enqueue = enqueue;
...
void ParallelSolution::genRepairBatchesForMultipleFailureRandom(
    vector<int> fail_node_ids, int num_agents, string scenario, int method) {
  filterFailedStripes(fail_node_ids);

  // Randomly shuffle the stripes to repair to form random batches
  random_device rd;
  mt19937 g(rd());
  shuffle(_stripes_to_repair.begin(), _stripes_to_repair.end(), g);
  ...
  for (int batchid = 0; batchid < _num_batches; batchid++) {
    ...
    for (int i = 0; i < _batch_size; i++) {
      ...
      genColoringForMultipleFailureLevelNew(curstripe, fail_node_ids, scenario,
                                            loadtable, 1);
...
//            int ret=hungary(rg_id, cur_match_stripe, _bipartite_matrix,
//            rg_id*_num_stripes_per_group*_num_agents, _node_belong);
int ParallelSolution::hungary(int rg_id, int cur_match_stripe, int *matrix,
                              int matrix_start_addr, int *node_selection) {
  int i;
  int ret;
  for (i = 0; i < _num_agents; i++) { // 遍历每个node
    if ((matrix[matrix_start_addr + cur_match_stripe * _num_agents + i] == 1) &&
        (_mark[i] == 0)) { // 该node是该stripe的存储节点
      // LOG << "   hungary " << cur_match_stripe << " check node " << i <<  "
      // node_selection " << " = " << node_selection[rg_id*_num_agents+i] <<
      // endl;
      _mark[i] = 1;
      int &selection = node_selection[rg_id * _num_agents + i];
      if (node_selection[rg_id * _num_agents + i] == -1) { // 该node尚未匹配
        if (_debug)
          LOG << "       hungary ret1.1: rg_id " << rg_id << ", mark (" << rg_id
              << ", " << i << ") = " << cur_match_stripe << endl;
        node_selection[rg_id * _num_agents + i] = cur_match_stripe;
        return 1;
      } else if ((ret = hungary(rg_id, node_selection[rg_id * _num_agents + i],
                                matrix, matrix_start_addr, node_selection)) ==
                 1) {
        node_selection[rg_id * _num_agents + i] = cur_match_stripe; // 尝试调整
        if (_debug)
          LOG << "       hungary ret1.2: rg_id " << rg_id << ", mark (" << rg_id
              << ", " << i << ") = " << cur_match_stripe << endl;
        return 1;
      }
    }
  ...
  if (_debug)
    LOG << "hungary: rg_id " << rg_id << ", cur_match_stripe "
        << cur_match_stripe << ", return 0" << endl;
  return 0;
}

int ParallelSolution::if_insert(Stripe *repairstripe, int rg_id,
                                int cur_match_stripe,
                                vector<int> soon_to_fail_node) {
  if (_debug) {
  ...
  int chunk_id;
  int *bak_node_belong =
      (int *)malloc(sizeof(int) * _num_agents); // clusterSize大小

  // 0. get out the nodeid of chunks in the repairstripe, and try to set
  // bipartite matrix 得到条带中的每个块所存储的nodeid
  ...
  for (int i = 0; i < nodeList.size(); i++) {
    ...
    if (_debug)
      LOG << node_id << " ";
    if (find(soon_to_fail_node.begin(), soon_to_fail_node.end(), node_id) !=
        soon_to_fail_node.end())
    ...
    _bipartite_matrix[rg_id * _num_stripes_per_group * _num_agents +
                      cur_match_stripe * _num_agents + node_id] = 1;
  }
  // LOG << endl;
  ...
  for (chunk_id = 0; chunk_id < helper.size(); chunk_id++) {
    ...
    // 返回0则表示失败
    int ret =
        hungary(rg_id, cur_match_stripe, _bipartite_matrix,
                rg_id * _num_stripes_per_group * _num_agents, _node_belong);
    if (ret == 0) {
      // LOG << "hungary = 0" << endl;
      break;
  ...
  if (chunk_id <= helper.size() - 1) {
    // LOG << "hungary rd_id = " << rg_id << " match_stripe" <<
    // repairstripe->getStripeId() << " false " << endl; reset the bipartite
    // matrix
    for (int node_id = 0; node_id < _num_agents; node_id++)
      _bipartite_matrix[rg_id * _num_stripes_per_group * _num_agents +
                        cur_match_stripe * _num_agents + node_id] = 0;
...
void ParallelSolution::update_bipartite_for_replace(
    int des_id, int stripe_id, int rg_id, int index_in_rg, string flag,
    vector<int> soon_to_fail_node) {
  ...
  for (int i = 0; i < nodelist.size(); i++) {
    ...
    // set bipartite matrix
    _bipartite_matrix[rg_id * _num_stripes_per_group * _num_agents +
                      index_in_rg * _num_agents + node_id] = bi_value;
  }
}

int ParallelSolution::replace(int src_id, int des_id, int rg_id, int *addi_id,
                              int num_related_stripes, string flag,
                              vector<int> soon_to_fail_node) {
  if (_debug)
    LOG << "replace src_id = " << src_id << ", des_id = " << des_id
        << ", rg_id = " << rg_id << endl;
  ...
  update_bipartite_for_replace(des_id, des_stripe_id, rg_id, index, "delete",
                               soon_to_fail_node); // 在二分图中删除该条带

  if (flag == "test_replace") {
    bakp_node_belong = (int *)malloc(sizeof(int) * _num_agents);
    memcpy(bakp_node_belong, _node_belong + rg_id * _num_agents,
           sizeof(int) * _num_agents);
  }
  ...
  if (flag == "test_replace") {
    if (ret == 0) {
      ...
      update_bipartite_for_replace(des_id, des_stripe_id, rg_id, index, "add",
                                   soon_to_fail_node);
      memcpy(_node_belong + rg_id * _num_agents, bakp_node_belong,
             sizeof(int) * _num_agents);
    ...
    for (i = src_id; i < num_related_stripes; i++) {
      ...
      ret = if_insert(cur_repair_stripe, rg_id, cur_stripe_num,
                      soon_to_fail_node); // 尝试将其他节点插入进去
      if (ret == 1) {
    ...
    for (i = rg_id * _num_stripes_per_group * _num_agents;
         i < (rg_id + 1) * _num_stripes_per_group * _num_agents; i++)
      _bipartite_matrix[i] = 0;

    for (i = 0; i < _cur_matching_stripe[rg_id]; i++) {
      stripe_id = _RepairGroup[rg_id * _num_stripes_per_group + i];
      update_bipartite_for_replace(stripe_id, _stripes_to_repair[stripe_id],
                                   rg_id, i, "add", soon_to_fail_node);
    }
    ...
    // update _RepairGroup
    i = 0;
    while (_RepairGroup[rg_id * _num_stripes_per_group + i] != des_id)
      i++;
    ...
    while (_record_stripe_id[i] != -1 && i < _num_stripes_per_group) {
      ...
      ret = if_insert(repstripe, rg_id, _cur_matching_stripe[rg_id],
                      soon_to_fail_node);
      ...
      // perform update
      _RepairGroup[rg_id * _num_stripes_per_group +
                   _cur_matching_stripe[rg_id]] = _record_stripe_id[i];
...
int ParallelSolution::greedy_replacement(int num_related_stripes,
                                         vector<int> soon_to_fail_node,
                                         int rg_id) {
  if (_debug)
    // LOG << "greedy_replacement.numstripes: " << num_related_stripes << ",
    // stfnode: " << soon_to_fail_node << ", rg_id : " << rg_id << endl;
    // 对当前rg进行optimize
    int i;
  ...
  int *addi_id = (int *)malloc(
      sizeof(int) *
      _num_stripes_per_group); // 未知, 大小为group中stripe数量上限
  best_src_id = -1;
  best_des_id = -1;
  if_benefit = 1;
  max_benefit = -1;

  memset(_record_stripe_id, -1,
         sizeof(int) * _num_stripes_per_group); // 记录当前group中有哪些stripe
  ...
  for (src_id = 0; src_id < num_related_stripes; src_id++) {
    ...
    for (int i = 0; i < _cur_matching_stripe[rg_id]; i++) {
      memset(addi_id, -1,
             sizeof(int) *
                 _num_stripes_per_group); // addi_id置为-1,
                                          // 记录加入到当前group的stripe
      des_id =
          _RepairGroup[rg_id * _num_stripes_per_group + i]; // 得到当前stripe_id

      string flag = "test_replace"; // 尝试进行交换
      // 交换: 拿出des_id, 插入src_id
      ret = replace(src_id, des_id, rg_id, addi_id, num_related_stripes, flag,
                    soon_to_fail_node);
      if (ret == 0) // 没有增益就继续
        continue;
      if (ret > 0) { // 只要产生增益,就退出对des_id的遍历
        ...
        best_des_id = des_id;
        max_benefit = ret;
        memcpy(_record_stripe_id, addi_id,
               sizeof(int) * _num_stripes_per_group);
        break;
      }
  ...
  // perform replacement
  if (max_benefit != -1) {
    ret = replace(best_src_id, best_des_id, rg_id, addi_id, num_related_stripes,
                  "perform_replace", soon_to_fail_node);
    if (_debug)
...
vector<RepairBatch *>
ParallelSolution::findRepairBatchs(vector<int> soon_to_fail_node,
                                   string scenario) {
  ...
  if (scenario == "scatter") {
    //_num_stripes_per_group =
    //(_num_agents-soon_to_fail_node.size())/(_ec->_n-2); //  -2 ???
    _num_stripes_per_group =
        (_num_agents - soon_to_fail_node.size()) / (_ec->_n);
  } else {
    // TODO
  ...
  _rg_num = (int)(ceil(_num_rebuilt_chunks * 1.0 / _num_stripes_per_group)) *
            _expand_ratio; // 所能接受的最大group数量
  ...
  _bipartite_matrix = (int *)malloc(sizeof(int) * _rg_num *
                                    _num_stripes_per_group * _num_agents);
  ...
  memset(_bipartite_matrix, 0,
         sizeof(int) * _num_stripes_per_group * _num_agents * _rg_num);
  ...
  while (true) {
    ...
    for (i = 0; i < num_related_stripes; i++) {
      ...
      if (_debug)
        LOG << endl
            << endl
            << "findReconstructionSets.stripe_id = " << stripe_id << endl;
      Stripe *repstripe = _stripe_list[stripe_id];
      ret = if_insert(repstripe, rg_index, _cur_matching_stripe[rg_index],
                      soon_to_fail_node);
      if (ret) {
        _RepairGroup[rg_index * _num_stripes_per_group +
                     _cur_matching_stripe[rg_index]] = i;
        _ifselect[i] = 1;
    ...
    while (ret)
      ret =
          greedy_replacement(num_related_stripes, soon_to_fail_node, rg_index);
    rg_index++;
...
// if st1 is better than st2, then return ture;
bool ParallelSolution::isBetter2(State st1, int color1, State st2, int color2,
                                 const vector<vector<int>> &table) {
  // just consider global input, because input load is easier to occur
  // imbanlance
  if (st1._load < st2._load) {
    return true;
  } else if (st1._load == st2._load) {
    if (st1._bdwt < st2._bdwt) {
      ...
      // LOG <<""<<endl;
      return false;
      // return (table[color1][0] < table[color2][0]) || (table[color1][0] ==
      // table[color2][0]); //全局better
    }
  }
  return false;
}

bool ParallelSolution::isBetter4(State st1, int color1, State st2, int color2,
                                 const vector<vector<int>> &table) {
  // just consider global input, because input load is easier to occur
  // imbanlance
  if (st1._load < st2._load) {
...
// if st1 is better than st2, then return ture;
bool ParallelSolution::isBetter3(State st1, int color1, State st2, int color2,
                                 const vector<vector<int>> &table) {
...
int ParallelSolution::chooseColor1(Stripe *stripe, vector<int> childColors,
                                   unordered_map<int, int> coloring, int idx) {
  // choose the color which has the minimum local load among the childColors
  // node.
  int bestColor = -1;
  ...
  for (auto newColor : childColors) // 对该节点邻居染色
  {
    ...
    vector<vector<int>> testTable =
        stripe->evaluateChange(_num_agents + _standby_size, idx, newColor);
...
int ParallelSolution::chooseColor2(Stripe *stripe, vector<int> childColors,
  ...
  for (auto newColor : childColors) // 对该节点邻居染色
  {
    ...
    vector<vector<int>> testTable =
        stripe->evaluateChange2(_num_agents + _standby_size, itm_idx, newColor);

    // State state = evalTable(testTable, childColors);
    State state = evalTable1(testTable);
    // LOG <<"DEBUG chooseColor2: after change idx :" << itm_idx[0] <<" color :
    // "<<newColor<<endl; LOG <<"load = "<<state._load<<endl; LOG <<"bdwt =
    // "<<state._bdwt<<endl; LOG <<"currTable:"<<endl; dumpTable(testTable);
...
int ParallelSolution::chooseColor2New(Stripe *stripe, vector<int> childColors,
                                      const vector<vector<int>> &loadTable,
                                      vector<int> itm_idx) {
  // choose the color which has the minimum local load among the childColors
  // node.
  ...
  for (auto newColor : childColors) // 对该节点邻居染色
  {
    ...
    //         stripeTable = stripe->evaluateChange(loadTable.size(),
    //         itm_idx_idx, newColor);
    //    }//这个写法有问题，没有加根节点的in

    stripeTable = stripe->evaluateChange2(loadTable.size(), itm_idx, newColor);

    // stripeTable = stripe->evaluateChange2(loadTable.size(), itm_idx,
    // newColor); //这个地方有无问题

    // vector<vector<int>> stripeTable =
    // stripe->evaluateChange2(loadTable.size(), itm_idx, newColor);
    ...
    // LOG <<"DEBUG chooseColor2New : after change idx :" << itm_idx[0] <<"
    // color : "<<newColor <<" placement : "<<placement <<endl; LOG <<"load =
    // "<<state._load<<endl; LOG <<"bdwt = "<<state._bdwt<<endl;
    // LOG<<"currTable:"<<endl;
    // dumpTable(currTable);
    // ZJL ADD
    if (isBetter2(state, newColor, bestState, bestColor,
                  currTable)) { // gloabal :3 非 ：2
      bestState = state;
      bestColor = newColor;
      // LOG<<"bestColor now is :"<<bestColor<<endl;
    }
...
int ParallelSolution::chooseColor4(Stripe *stripe,
                                   const vector<int> &childColors,
                                   const vector<vector<int>> &loadTable,
                                   int idx) {
  ...
  for (auto newColor : childColors) // 对该节点邻居染色
  {
    ...
    vector<vector<int>> stripeTable =
        stripe->evaluateChange(loadTable.size(), idx, newColor);
    vector<vector<int>> currTable = loadTable;
    for (int i = 0; i < loadTable.size(); i++) {
      currTable[i][0] += stripeTable[i][0];
      currTable[i][1] += stripeTable[i][1];
...
void ParallelSolution::SingleMLP(Stripe *stripe, const vector<int> &itm_idx,
                                 const vector<int> &candidates, ECDAG *ecdag,
                                 unordered_map<int, int> &coloring) {
  LOG << "ParallelSolution::SingleMLP for stripe " << stripe->getStripeId()
      << endl;
  ...
  // ecdag->dumpTOPO();
  LOG << "load = " << stripe->getLoad() << endl
      << "bdwt =  " << stripe->getBdwt() << endl;
  return;
}

void ParallelSolution::SingleMLPAll(Stripe *stripe, const vector<int> &itm_idx,
                                    const vector<int> &candidates, ECDAG *ecdag,
                                    unordered_map<int, int> &coloring,
                                    int num_agents) {
  // LOG << "ParallelSolution::SingleMLP for stripe " << stripe->getStripeId()
  ...
  for (int i = 0; i < topoIdxs.size(); i++) {
    ...
    for (int i = 0; i < _num_agents; i++) {
      if (find(stripe->_fail_blk_idxs.begin(), stripe->_fail_blk_idxs.end(),
               i) == stripe->_fail_blk_idxs.end())
  ...
  // ecdag->dumpTOPO();
  // LOG << "load = " << stripe->getLoad() << endl << "bdwt =  " <<
  // stripe->getBdwt() << endl;
  return;
}

bool ParallelSolution::areAllNodesInLeaves(
    const std::vector<int> &node_parents,
    const std::vector<int> &ecdage_leaves) {
  std::unordered_set<int> leaves_set(ecdage_leaves.begin(),
                                     ecdage_leaves.end());
...
void ParallelSolution::SingleMLPGroup(
    Stripe *stripe, const vector<int> &itm_idx, const vector<int> &candidates,
    ECDAG *ecdag, unordered_map<int, int> &coloring, int num_agents,
    unordered_map<string, vector<int>> itm_idx_info) {
  LOG << "ParallelSolution::SingleMLPGroup for stripe " << stripe->getStripeId()
      << endl;
  ...
  for (int i = 0; i < topoIdxs.size(); i++) {
    ...
    vector<int> itm_idx =
        ecdag->getECNodeMapNew()[itm_childidx[0]]->getParentIndices();
...
void ParallelSolution::SingleMLPLevel(
    Stripe *stripe, const vector<int> &itm_idx, const vector<int> &candidates,
    ECDAG *ecdag, unordered_map<int, int> &coloring, int num_agents,
    unordered_map<string, vector<int>> itm_idx_info) {
  LOG << "ParallelSolution::SingleMLPLevel for stripe " << stripe->getStripeId()
      << endl;
  ...
  for (int i = 0; i < topoIdxs.size(); i++) {
    ...
    if (areAllNodesInLeaves(node_child, ecdage_leaves)) {
      ...
      int bestColor =
          chooseColor2(stripe, childColors, coloring, itm_idx); // New ： global
      ...
      for (auto it : itm_idx) {
        // LOG<<"913hang:"<<it<<endl;
        stripe->changeColor(it, bestColor);
        // LOG <<"AllNodesInLeaves DEBUG : after change idx :" << it <<" color :
        // "<<bestColor<<endl;
        coloring[it] = bestColor;
      }
    } else {
      ...
      for (auto it : itm_idx) {
        ...
        stripe->changeColor(it, bestColor);
        // LOG <<"NoAllNodesInLeaves DEBUG : after change idx :" << it <<" color
        // : "<<bestColor<<endl;
        coloring[it] = bestColor;
      }
    }
...
void ParallelSolution::GloballyMLP(Stripe *stripe, const vector<int> &itm_idx,
                                   const vector<int> &candidates, ECDAG *ecdag,
                                   unordered_map<int, int> &coloring,
...
void ParallelSolution::GloballyMLP_yh(Stripe *stripe,
                                      const vector<int> &itm_idx,
                                      unordered_map<int, int> &coloring,
                                      vector<vector<int>> &loadTable) {
...
int ParallelSolution::chooseColor_fullnode(Stripe *stripe,
                                           const vector<int> &childColors,
                                           const vector<vector<int>> &loadTable,
                                           int idx) {
  ...
  for (auto newColor : childColors) {
    ...
    State state = evalTable1(currTable); // eval gloabl load
    // LOG <<"DEBUG chooseColor_fullnode: after change idx :" << idx <<"color :
    // "<<newColor<<endl; LOG <<"load = "<<state._load<<endl; LOG <<"bdwt =
    // "<<state._bdwt<<endl;
...
void ParallelSolution::genParallelColoringForSingleFailure(
    Stripe *stripe, int fail_node_id, int num_agents, string scenario,
    vector<vector<int>> &loadTable) {
...
void ParallelSolution::genColoringForMultipleFailure(
    Stripe *stripe, unordered_map<int, int> &res, vector<int> fail_node_ids,
    int num_agents, string scenario, vector<int> &placement, int greedy) {
  // map a sub-packet idx to a real physical node id
  ...
  vector<int> candidates;
  if (scenario == "standby") {
    LOG << "1216 hang  ParallelSolution::genColoringForMultipleFailure start!"
        << endl;
    // exit(1);
    //  for each stripe
    int failnum_count = 0;
    for (auto concact_idx : ecdag->_ecConcacts) {
      ...
      failnum_count++;
      // LOG << "1318 hang DEBUG concact " << concact_idx << " choose " <<
      // repair_node_id << endl;
    }
  } else {
    cout << "1186 hang" << endl;
  ...
  // //分层去染色
  // for(auto it:itm_idx){
  //     vector<int> parent = ecNodeMap[it]->getChildIndices();  //往下看的 根
  //     32767 32766 LOG<<it<<"  :  " ; for(auto it_p:parent){
  //         LOG<<it_p<<"   ";
  //     }
  //     LOG<<endl;
  // }
...
void ParallelSolution::genColoringForMultipleFailureLevel(
    Stripe *stripe, unordered_map<int, int> &res, vector<int> fail_node_ids,
    int num_agents, string scenario, vector<int> &placement, int greedy) {
  // map a sub-packet idx to a real physical node id
  // LOG << "ParallelSolution::genColoringForMultipleFailureLevel start" <<
  // endl; assert(_ec->_n + fail_node_ids.size() <= num_agents);
  _num_agents = num_agents;
  ...
  vector<int> candidates;
  if (scenario == "standby") {
    // LOG << "1463 hang  ParallelSolution::genColoringForMultipleFailureLevel
    // start!"<<endl;
    ...
    for (auto concact_idx : ecdag->_ecConcacts) {
      ...
      res.insert(make_pair(concact_idx, repair_node_id));
      // LOG << "1484hang DEBUG concact " << concact_idx << " choose " <<
      // repair_node_id << endl;
    }
  } else {
    _standby_size = 0;
    ...
    for (auto concact_idx : ecdag->_ecConcacts) {
      ...
      res.insert(make_pair(concact_idx, repair_node_id));
      // LOG << "1505 hang DEBUG concact " << concact_idx << " choose " <<
      // repair_node_id << endl; cout << "[INFO] stripe " <<
      // stripe->getStripeId() << " fail nodeidx is " << (32767-concact_idx)<< "
      // choose new node " << repair_node_id <<"scatter"<<endl;
    }
  }
  ...
  // cout << "1563 hang candidates_color:num_agents,"<<num_agents<<"
  // _standby_size: "<<_standby_size<<endl;

  for (int i = 0; i < num_agents + _standby_size; i++) {
    if (find(fail_node_ids.begin(), fail_node_ids.end(), i) ==
        fail_node_ids.end()) {
      if (find(avoid_node_ids.begin(), avoid_node_ids.end(), i) ==
          avoid_node_ids.end()) {
  ...
  if (scenario == "standby") {
    ...
    if (greedy == 1) {
      // SingleMLPAll(stripe,itm_idx, candidates, ecdag, res,_num_agents);
      SingleMLPLevel(stripe, itm_idx, candidates_color, ecdag, res, _num_agents,
                     itm_idx_info);
      // GloballyMLPLevel(stripe,itm_idx,res,candidates_color,loadTable);
    } else if (greedy == 0) {
      SingleMLP(stripe, itm_idx, candidates_color, ecdag, res);
    } else if (greedy == 2) {
      SingleMLPGroup(stripe, itm_idx, candidates_color, ecdag, res, _num_agents,
                     itm_idx_info);
    }
  } else {
    // SingleMLP(stripe,itm_idx, candidates, ecdag, res);
    if (greedy == 1) {
      // SingleMLPAll(stripe,itm_idx, candidates, ecdag, res,_num_agents);
      SingleMLPLevel(stripe, itm_idx, candidates, ecdag, res, _num_agents,
                     itm_idx_info);
    } else if (greedy == 0) {
      SingleMLP(stripe, itm_idx, candidates, ecdag, res);
    } else if (greedy == 2) {
      SingleMLPGroup(stripe, itm_idx, candidates, ecdag, res, _num_agents,
                     itm_idx_info);
    }
  }
}
...
// TODO: multiple ecdag coloring for level
void ParallelSolution::genColoringForMultipleFailureLevelGlobal(
    Stripe *stripe, unordered_map<int, int> &res, vector<int> fail_node_ids,
    string scenario, vector<vector<int>> &loadTable, int method) {
  // map a sub-packet idx to a real physical node id
  // LOG << "ParallelSolution::genColoringForMultipleFailureLevel start" <<
  // endl; assert(_ec->_n + fail_node_ids.size() <= num_agents);
  ...
  if (scenario == "standby") {
    LOG << "1687 hang  "
           "ParallelSolution::genColoringForMultipleFailureLevelGlobal standby "
           "start!"
        << endl;
    ...
    for (auto concact_idx : ecdag->_ecConcacts) {
      ...
      res.insert(make_pair(concact_idx, repair_node_id));
      // LOG << "1716 hang DEBUG concact " << concact_idx << " choose " <<
      // repair_node_id << endl;
    }
  } else {
    _standby_size = 0;
    ...
    for (auto concact_idx : ecdag->_ecConcacts) {
      ...
      // repair_node_id_concact.push_back(repair_node_id);
      res.insert(make_pair(concact_idx, repair_node_id));
      // LOG << "1737 hang DEBUG concact " << concact_idx << " choose " <<
      // repair_node_id << endl;
    }
  }
  // intermediate node idxs
  ...
  vector<int> candidates_color;
  cout << "1777 hang candidates_color:num_agents," << _agents_num
       << " _standby_size: " << _standby_size << endl;
  ...
  if (scenario == "standby") {
    ...
    if (method == 1) {
      // SingleMLPAll(stripe,itm_idx, candidates, ecdag, res,_num_agents);
      // SingleMLPLevel(stripe,itm_idx, candidates_color, ecdag,
      // res,_num_agents,itm_idx_info);
      GloballyMLPLevel(stripe, itm_idx, res, candidates_color, loadTable);
    } else if (method == 0) {
      SingleMLP(stripe, itm_idx, candidates_color, ecdag, res);
    } else if (method == 2) {
...
void ParallelSolution::genColoringForMultipleFailureLevelNew(
    Stripe *stripe, vector<int> fail_node_ids, string scenario,
    vector<vector<int>> &loadTable, int method) {
  ...
  //     if (find(fail_node_ids.begin(), fail_node_ids.end(), i) ==
  //     fail_node_ids.end()){
  //         if(candidates.size() < stripe ->_nodelist.size())   //怀疑是agent
  //         数量
  //             candidates.push_back(i);
  //             //你这样的话就全是小的了，含没有参与进来的 后面的agent
  //     }
  // }

  for (int i = 0; i < stripe->_nodelist.size(); i++) {
    if (find(fail_node_ids.begin(), fail_node_ids.end(),
             stripe->_nodelist[i]) == fail_node_ids.end()) {
  ...
  // for(int i = 0 ; i < (ecdag ->_ecHeaders).size();
  // i++){//仅根节点的，验证一下
...
void ParallelSolution::GloballyMLPLevel(Stripe *stripe,
                                        const vector<int> &itm_idx,
                                        unordered_map<int, int> &coloring,
                                        const vector<int> &candidates,
                                        vector<vector<int>> &loadTable) {
  // LOG << "[DEBUG] GloballyMLPLevel MLP: stripe " << stripe->getStripeId() <<
  // endl;
  ...
  for (int i = 0; i < topoIdxs.size(); i++) {
    ...
    if (oldColor != -1) { // 在上一阶段已经被染色过了
      // cout<<"[info]1606 hang idx"<<idx<<"has been colored : "<< oldColor
      // <<endl;
      continue;
    }
    // 分类型，第一层只传孩子结点颜色
    ...
    // candidates_coloring.insert(candidates_coloring.end(),childColors.begin(),childColors.end());
    // //应该是只要根节点和孩子节点颜色，不需要那么多

    // reverse(childColors.begin(),childColors.end());

    // 根据idx 去找 itm_idx
    vector<int> itm_childidx = node->getChildIndices();
    vector<int> itm_idx_parents =
        ecdag->getECNodeMapNew()[itm_childidx[0]]->getParentIndices();
    ...
    if (areAllNodesInLeaves(node_child, ecdage_leaves)) { // 第一层
      ...
      // }
      int bestColor =
          chooseColor2New(stripe, childColors, loadTable, itm_idx_parents);
      ...
      int bestColor = chooseColor2New(stripe, candidates_coloring, loadTable,
                                      itm_idx_parents);
      ...
      for (auto it : itm_idx_parents) {
        ...
        coloring[it] = bestColor;
        // LOG <<"NoAllNodesInLeaves DEBUG : after change idx :" << it <<" color
        // : "<<bestColor<<" placement : "<<placement<<endl;
      }
    }
  }
  // 3. add into loadtable
...
void ParallelSolution::genParallelColoringForMultipleFailure(
    Stripe *stripe, vector<int> fail_node_ids, int num_agents, string scenario,
    vector<vector<int>> &loadTable) {
  // map a sub-packet idx to a real physical node id
  // LOG << "ParallelSolution::genParallelColoringForMultipleFailure start" <<
  // endl;
  _num_agents = num_agents;
  ECDAG *ecdag = stripe->getECDAG();
  unordered_map<int, int> res;
  ...
  if (scenario == "standby") {
    ...
    LOG << "1668 hang  ParallelSolution::genParallelColoringForMultipleFailure "
           "start!"
        << endl;
    ...
    for (auto concact_idx : ecdag->_ecConcacts) {
      ...
      res.insert(make_pair(concact_idx, repair_node_id));
      // LOG << "2047 hang DEBUG concact " << concact_idx << " choose " <<
      // repair_node_id << endl;
    }
  }
    ...
    for (auto concact_idx : ecdag->_ecConcacts) {
      ...
      res.insert(make_pair(concact_idx, repair_node_id));
      if (DEBUG_ENABLE)
        LOG << "2067 hang DEBUG concact " << concact_idx << " choose "
            << repair_node_id << endl;
    }
  }
...
void ParallelSolution::genOfflineColoringForSingleFailure(
    Stripe *stripe, int fail_node_id, int num_agents, string scenario,
    vector<vector<int>> &loadTable, vector<int> &placement) {
  ...
  if (scenario == "standby")
    ...
    LOG << "stripe" << stripe->getStripeId()
        << " choose the new_node: " << repair_node_id << endl;
  }
  stripe->_new_node = repair_node_id;
  res.insert(make_pair(ecHeaders[0], repair_node_id));
  ...
  // LOG << "itm_idx.size: " << itm_idx.size() << ", itm_offline_coloring.size:
  // " << itm_offline_coloring.size() << endl;
...
void ParallelSolution::genColoringForSingleFailure(
    Stripe *stripe, unordered_map<int, int> &res, int fail_node_id,
    int num_agents, string scenario, vector<int> &placement) {
...
vector<vector<int>> genTable(unordered_map<int, int> out,
                             unordered_map<int, int> in, int _num_agents) {
...
void dumpPlacement(vector<RepairBatch *> batch_list) {
  for (auto batch : batch_list) {
    ...
    LOG << " load=" << batch->getLoad() << " avgload = "
        << batch->getLoad() * 1.0 / batch->getStripeList().size();
    LOG << endl;
  }
}
...
void ParallelSolution::improve(int fail_node_id, string scenario) {
  ...
  unordered_map<int, vector<int>>
      mem_has_tryed; // batch->tryed_stripe_list: for avoid the repeat computing
                     // if the stripe could insert this batch
  ...
  for (int i = 0; i < _batch_list.size(); i++) {
    ...
    for (int i = 0; i < full_stripe_list.size(); i++) {
      ...
      unordered_map<int, int> coloring;
      genColoringForSingleFailure(stripe, coloring, fail_node_id, _num_agents,
                                  scenario, placement);
      // genOfflineColoringForSingleFailure(stripe, fail_node_id, _num_agents,
      // scenario, loadTable, placement);
      stripe->setColoring(coloring);
      stripe->evaluateColoring();
    }
  ...
  while (true) {
    ...
    sort(_batch_list.begin(), _batch_list.end(),
         [](RepairBatch *batch1, RepairBatch *batch2) {
           return batch1->getLoad() / batch1->getStripeList().size() <
                  batch2->getLoad() / batch2->getStripeList().size();
         });
    bool all_optimized = true;
    vector<pair<Stripe *, RepairBatch *>>
        log; // for insert entire tail batch into pre batch
    ...
    for (auto tail_stripe : stripe_list) {
      ...
      for (int i = 0; i < last_batch_id; i++) {
        ...
        if (find(mem.begin(), mem.end(), tail_stripe->getStripeId()) !=
            mem.end()) {
          continue;
        }
        mem_has_tryed[batch->getBatchId()].push_back(
            tail_stripe->getStripeId());
        debug_try_num++;
        ...
        unordered_map<int, int> coloring;
        genParallelColoringForSingleFailure(tail_stripe, fail_node_id,
                                            _num_agents, scenario, loadTable);
        ...
        // LOG << "try insert stripe " << tail_stripe->getStripeId() << " into
        // batch " << batch->getBatchId()    << endl; LOG << "avg load pre = "
        // << loadpre << "/" << stripe_num << " = " << avgpre << endl; LOG <<
        // "avg load after = " << loadafter << "/" << stripe_num+1 << " = " <<
        // avgafter << endl;

        // matching batch and stripe
        if (avgafter < avgpre) {
          // LOG << "match insert stripe " << tail_stripe->getStripeId() << "
          // into batch " << batch->getBatchId()    << endl; LOG << "avg load
          // pre = " << loadpre << "/" << stripe_num << " = " << avgpre << endl;
          // LOG << "avg load after = " << loadafter << "/" << stripe_num+1 << "
          // = " << avgafter << endl;
          batch_idx = i;
      ...
      // 2. this stripe has no prebatch to insert, then the improve progress
      // will stop
      if (batch_idx == -1) {
        all_optimized = false;
        break;
    ...
    for (auto pair : log) {
      ...
      LOG << "perform insert stripe " << stripe->getStripeId() << " into batch "
          << batch->getBatchId() << endl;
      batch->push(stripe);
    ...
    gettimeofday(&time2, NULL);
    LOG << "duration " << DistUtil::duration(time1, time2)
        << " for try matching " << debug_try_num << " times " << endl;
...
void ParallelSolution::improve_enqueue(int fail_node_id, string scenario) {
  ...
  while (true) {
    ...
    if (_batch_list.size() == 0)
      break;
    // sort(_batch_list.begin(), _batch_list.end(), [](RepairBatch* batch1,
    // RepairBatch* batch2){
    //     return batch1->getLoad()/batch1->getStripeList().size() <
    //     batch2->getLoad()/batch2->getStripeList().size();
    // });
    ...
    float best_avg = evalTable1(head_batch->getLoadTable(_num_agents))._load *
                     1.0 / stripe_num;
    ...
    vector<vector<int>> loadTable =
        head_batch->getLoadTable(_num_agents); // contain the cache stripe load
    ...
    while (true) {
      ...
      for (int i = tail_batch_idx; i > 0; i--) {
        ...
        for (auto tail_stripe : tail_batch->getStripeList()) {
          if (find(stripeIdx_to_insert.begin(), stripeIdx_to_insert.end(),
                   tail_stripe->getStripeId()) != stripeIdx_to_insert.end())
            continue;
          // remove source nodes that we should avoid
          ...
          if (new_node_load * 1.0 / (stripe_num + 1) > best_avg) {
            if (DEBUG_ENABLE)
              LOG << "stripe" << tail_stripe->getStripeId()
                  << " new_node input load is to large, = " << new_node_load
                  << " avg=" << new_node_load * 1.0 / (stripe_num + 1) << endl;
            continue;
          }
          ...
          source_node_ids.erase(find(source_node_ids.begin(),
                                     source_node_ids.end(), fail_node_id));
          ...
          if (DEBUG_ENABLE)
            LOG << "output avg = " << max_gloabl_output << "/"
                << (stripe_num + 1) << "=" << avg_output << endl;
      ...
      if (!find_stripe) { // search all stripe
        LOG << "search all stripe, curr batch size = "
            << head_batch->getStripeList().size() << endl;
        break;
      }
      ...
      genParallelColoringForSingleFailure(stripe, fail_node_id, _num_agents,
                                          scenario, loadTable);
      ...
      // dump
      LOG << "insert stripe " << stripe->getStripeId() << " into batch "
          << head_batch->getBatchId()
          << " coloring duration = " << DistUtil::duration(time1, time2)
          << endl;
      LOG << "best avg load = "
          << evalTable1(head_batch->getLoadTable(_num_agents + _standby_size))
                 ._load
          << "/" << head_batch->getStripeList().size() << " = " << best_avg
          << endl;
      ...
      LOG << curr_time / 1000 << " " << stripe_num << " " << loadafter / _ec->_w
          << " " << bdwtafter / _ec->_w << " " << avgafter << endl;
      if (avgafter > best_avg) {
        ...
        for (auto it : batch_stripe) {
          ...
          if (batch_to_delete->getStripeList().size() == 0) {
            _batch_list.erase(
                find(_batch_list.begin(), _batch_list.end(), batch_to_delete));
          }
        }
        batch_stripe.clear();
    ...
    // dump
    LOG << "Improve batch" << head_batch->getBatchId() << " for "
        << DistUtil::duration(improve1, improve2) << endl;
    int avgload = head_batch->getLoad() / head_batch->getStripeList().size();
    LOG << "avg load = " << head_batch->getLoad() << "/"
        << head_batch->getStripeList().size() << " = " << avgload << endl;
  }
  return;
}

void ParallelSolution::improve_multiple(vector<int> fail_node_ids,
  ...
  for (int i = 0; i < _batch_list.size(); i++) {
    ...
    for (int i = 0; i < full_stripe_list.size(); i++) {
      ...
      unordered_map<int, int> coloring;
      // genColoringForMultipleFailure(stripe, coloring, fail_node_ids,
      // _num_agents, scenario, placement,0);
      ...
      //     genColoringForSingleFailure(stripe, coloring, fail_node_ids[0],
      //     _num_agents, scenario, placement);
      // }else{
      genColoringForMultipleFailureLevel(stripe, coloring, fail_node_ids,
                                         _num_agents, scenario, placement,
                                         method);
      //            }
  ...
  while (true) {
    ...
    //         return batch1->getStripeList().size() >
    //         batch2->getStripeList().size();
    //     }
    ...
    vector<pair<RepairBatch *, Stripe *>> batch_stripe;
    vector<int> stripeIdx_to_insert;
    vector<vector<int>> loadTable = head_batch->getLoadTable(
        _num_agents + _standby_size); // contain the cache stripe load
    ...
    while (true) {
      ...
      if (head_batch->getStripeList().size() + batch_stripe.size() >=
          _batch_size) {
        if (DEBUG_ENABLE)
          LOG << "Batch full! Current size: "
              << head_batch->getStripeList().size() + batch_stripe.size()
      ...
      for (int i = tail_batch_idx; i > 0; i--) {
        ...
        for (auto tail_stripe : tail_batch->getStripeList()) {
          ...
          for (auto failnodeid : fail_node_ids) {
            auto it = find(source_node_ids.begin(), source_node_ids.end(),
                           failnodeid);
      ...
      // dumpTable(loadTable);
      genParallelColoringForMultipleFailure(stripe, fail_node_ids, _num_agents,
                                            scenario, loadTable);
      // stripe->dumpLoad(_num_agents);
...
void ParallelSolution::improve_multipleNew(vector<int> fail_node_ids,
                                           string scenario, int method) {
  // init
  if (DEBUG_ENABLE)
    LOG << "ParallelSolution::improve_multiple start" << endl;
...
void ParallelSolution::improve_hybrid(int fail_node_id, string scenario) {
  ...
  gettimeofday(&time1, NULL);
  for (int i = 0; i < _batch_list.size(); i++) {
    // LOG << "[DEBUG] coloring batch: " << i << "/" << _batch_list.size() <<
    // endl;
    RepairBatch *batch = _batch_list[i];
    vector<vector<int>> loadTable(_num_agents, {0, 0}); // {out,in}
    ...
    for (int i = 0; i < full_stripe_list.size(); i++) {
      Stripe *stripe = full_stripe_list[i];
      ECDAG *ecdag = stripe->genRepairECDAG(_ec, {fail_node_id});
      // genColoringForSingleFailure(stripe, coloring, fail_node_id,
      // _num_agents, scenario, placement);
      genOfflineColoringForSingleFailure(stripe, fail_node_id, _num_agents,
                                         scenario, loadTable, placement);
    }
  ...
  while (true) {
    ...
    while (true) {
      if (_batch_request &&
          batch_stripe.size() + head_batch->getStripeList().size() >=
              _batch_size / 2) {
        break;
      }
      // insert one stripe into one head_batch
      ...
      for (int i = tail_batch_idx; i > 0; i--) {
        ...
        for (auto tail_stripe : tail_batch->getStripeList()) {
          ...
          if (avg_output > best_avg) {
            LOG << "source node output load is to large" << max_output << "/"
                << stripe_num + 1 << "=" << avg_output << ", pass" << endl;
            debug_pass_output++;
            continue;
          }
          find_stripe = true;
      ...
      if (batch_stripe.size() + head_batch->getStripeList().size() <
          _batch_size / 2) {
        genOfflineColoringForSingleFailure(stripe, fail_node_id, _num_agents,
                                           scenario, loadTable, placement);
      } else {
      ...
      if (DEBUG_ENABLE) {
        LOG << "select candidate duration: " << select_candidate
            << " , and pass intput:" << debug_pass_input
            << ", pass output:" << debug_pass_output << endl;
        LOG << "insert stripe " << stripe->getStripeId() << " into batch "
            << head_batch->getBatchId() << "  duration: " << try_merge << endl;
        LOG << "best avg load = "
            << evalTable1(head_batch->getLoadTable(_num_agents + _standby_size))
                   ._load
            << "/" << head_batch->getStripeList().size() << " = " << best_avg
    ...
    if (DEBUG_ENABLE) {
      LOG << "Improve batch" << head_batch->getBatchId() << " for "
          << DistUtil::duration(improve1, improve2) << endl;
      float avgload =
          head_batch->getLoad() * 1.0 / head_batch->getStripeList().size();
      LOG << "avg load = " << head_batch->getLoad() << "/"
          << head_batch->getStripeList().size() << " = " << avgload << endl;
    }
  }
...
void ParallelSolution::improve_hungary(int fail_node_id, string scenario) {
  ...
  while (true) {
    ...
    //         for(auto tail_stripe : tail_batch->getStripeList())
    //         {
    //             if(find(stripeIdx_to_insert.begin(),
    //             stripeIdx_to_insert.end(), tail_stripe->getStripeId()) !=
    //             stripeIdx_to_insert.end())
    //                 continue;
    //             // remove source nodes that we should avoid
    //             vector<int> avoid_node_ids = tail_stripe->getPlacement();
    //             vector<int> candidates;
    ...
    //             int new_node_load = loadTable[min_load_color][1] + _ec->_w;
    //             if(new_node_load*1.0/(stripe_num+1) > best_avg){
    //                 LOG << "new_node input load is to large, = " <<
    //                 new_node_load << " avg=" <<
    //                 new_node_load*1.0/(stripe_num+1) << endl; continue;
    //             }
    //             // choose the maximum outputload amoung stripe source_node
    //             int max_load = 0;
    //             int max_load_color = -1;
    //             vector<int> source_node_ids = tail_stripe->getPlacement();
    //             source_node_ids.erase(find(source_node_ids.begin(),
    //             source_node_ids.end(), fail_node_id)); for(auto it :
    //             source_node_ids)
    //             {
    //                 int outLoad = loadTable[it][0];
    //                 if(outLoad > max_load)
    ...
    //             // LOG << "output avg = " << max_gloabl_output << "/" <<
    //             (stripe_num+1) << "=" << avg_output << endl; if(avg_output >
    //             best_avg){
    //                 // LOG << "soucenode output load is to large" << endl;
    //                 continue;
    //             }
    ...
    //     gettimeofday(&time1, NULL);
    //     batch_stripe.push_back(make_pair(batch,stripe));
    //     genParallelColoringForSingleFailure(stripe, fail_node_id,
    //     _num_agents, scenario, loadTable);
    //     // calculate average load after insert tail stripe
    ...
    //     << endl;
    //     // LOG << "best avg load = " <<
    //     evalTable(head_batch->getLoadTable(_num_agents))._load << "/" <<
    //     head_batch->getStripeList().size() << " = " << best_avg << endl;
    //     // LOG << "batch.load = " << head_batch->getLoad() << endl;
    //     // LOG << "avg load after = " << loadafter << "/" << stripe_num+1 <<
    //     " = " << avgafter << endl; stripe_num++; if(avgafter > best_avg){
    //         // cache
    //         stripeIdx_to_insert.push_back(stripe->getStripeId());
    //     }else{
    //         // perform merge
    ...
    //             if(batch_to_delete->getStripeList().size() == 0)
    //             {
    //                 _batch_list.erase(find(_batch_list.begin(),
    //                 _batch_list.end(), batch_to_delete));
    //             }
    //         }
    //         batch_stripe.clear();
    //         stripeIdx_to_insert.clear();
...
State ParallelSolution::evalTable(vector<vector<int>> table,
                                  vector<int> colors) {
...
Stripe *ParallelSolution::choose(vector<bool> flags,
...
void ParallelSolution::genRepairBatchesForSingleFailure(int fail_node_id,
                                                        int num_agents,
                                                        string scenario) {

  LOG << "ParallelSolution::genRepairBatchesForSingleFailure.fail_node_id = "
      << fail_node_id << endl;
  ...
  LOG << "ParallelSolution::genRepairBatchesForSingleFailure.stripes to "
         "repair: "
      << _stripes_to_repair.size() << endl;

  // 1. We first perform Hungarian algorithm to divide stripes into batches
  // we also sort batches based on the number of stripes in a batch in
  // descending order
  _num_agents = num_agents;
  ...
  //     for(auto idx : it->getPlacement())
  //     {
  //         if(find(placement.begin(), placement.end(), idx) ==
  //         placement.end())
  //             placement.push_back(idx);
  //     }
  // }
  ...
  //     ECDAG* ecdag = stripe->genRepairECDAG(_ec, fail_node_id);
  //     unordered_map<int, int> coloring;
  //     genOfflineColoringForSingleFailure(stripe, coloring, fail_node_id,
  //     _num_agents, scenario, loadTable, placement);
  // }
  // _batch_queue.push(head_batch);
  ...
  // 2. We try to insert stripes of latter batches into previous batches to
...
void ParallelSolution::genRepairBatchesForMultipleFailure(
    vector<int> fail_node_ids, int num_agents, string scenario, int method) {
  // if(DEBUG_ENABLE)
  ...
  if (DEBUG_ENABLE) {
    ...
    LOG << "ParallelSolution::genRepairBatchesForMultipleFailure.stripes to "
           "repair: "
        << _stripes_to_repair.size() << endl;
  }
...
void ParallelSolution::genRepairBatchesForMultipleFailureNew(
    vector<int> fail_node_ids, int num_agents, string scenario, int method) {
  // if(DEBUG_ENABLE)
  // LOG << "ParallelSolution::genRepairBatchesForMultipleFailureNew begin" <<
  // endl; cout << "[INFO] fail_node_id = " << fail_node_ids << endl;
  //  0. we first figure out stripes that stores a block in $fail_node_id
  filterFailedStripes(fail_node_ids);
  // cout << "[INFO] stripes to repair: " << _stripes_to_repair.size() << endl;
  vector<Stripe *> stripes_to_repair_vec;
  ...
  if (DEBUG_ENABLE) {
    ...
    LOG << "ParallelSolution::genRepairBatchesForMultipleFailureBatch.stripes "
           "to repair: "
        << _stripes_to_repair.size() << endl;
  }
  ...
  for (int batchid = 0; batchid < _num_batches; batchid++) {
    ...
    for (int i = 0; i < _batch_size; i++) {
      ...
      // genColoringForMultipleFailureLevel(curstripe, coloring, fail_node_ids,
      // num_agents, scenario,nodeidlist ,method);
      // genColoringForMultipleFailureLevelGlobal(curstripe, coloring,
      // fail_node_ids,scenario,loadtable ,method); //zhangJL add LOG << "[INFO]
      // INIT STRIPE= " << stripeidx << " fail_num = " <<
      // curstripe->_fail_blk_idxs.size() << endl;

      // method = 1;
      // if (curstripe ->_fail_blk_idxs.size() == 2 || curstripe
      // ->_fail_blk_idxs.size() == 1) {
      //     method = 0 ;
      // }
      if (method == 1) {
        int load_hyper = 0;
      ...
      // genBalanceColoringForSingleFailure(curstripe, fail_node_id, scenario,
      // loadtable);
    }
    RepairBatch *curbatch = new RepairBatch(batchid, cur_stripe_list);
    curbatch->evaluateBatch(_cluster_size);
...
void ParallelSolution::genRepairBatchesForMultipleFailureNewTest(
    vector<int> fail_node_ids, int num_agents, string scenario, int method) {
  LOG << "ParallelSolution::genRepairBatchesForMultipleFailureNew begin"
      << endl;
  ...
  std::unordered_map<Stripe *, vector<vector<int>>>
      stripe_dummy_loads; // 缓存试跑的 in/out 拓扑图

  for (auto stripe : stripes_to_repair_vec) {
    ECDAG *curecdag = stripe->genRepairECDAG(_ec, fail_node_ids);
    stripe->refreshECDAG(curecdag);

    vector<vector<int>> dummy_loadtable =
        vector<vector<int>>(_cluster_size, {0, 0});
    genColoringForMultipleFailureLevelNew(stripe, fail_node_ids, scenario,
                                          dummy_loadtable, 1);

    stripe_dummy_loads[stripe] =
        dummy_loadtable; // 存下这个条带试跑后的完整节点开销
  ...
  vector<vector<vector<int>>> batch_node_loads(
      _num_batches, vector<vector<int>>(_cluster_size, {0, 0}));
  ...
  for (size_t i = 0; i < stripes_to_repair_vec.size(); ++i) {
    ...
    int min_bottleneck =
        INT_MAX; // 我们希望放入后，整个 batch 的最高水位线尽可能低

    for (int b = 0; b < _num_batches; ++b) {
      if (balanced_batches[b].size() >= _batch_size)
        continue;

      int potential_batch_bottleneck = 0;
      for (int node_id = 0; node_id < _cluster_size; ++node_id) {
        // 虚拟推演：假设放进这个 Batch，该节点的 in 和 out 会涨到多少
        int sim_in = batch_node_loads[b][node_id][0] +
                     stripe_dummy_loads[cur_s][node_id][0];
        int sim_out = batch_node_loads[b][node_id][1] +
                      stripe_dummy_loads[cur_s][node_id][1];
    ...
    for (int node_id = 0; node_id < _cluster_size; ++node_id) {
      batch_node_loads[best_batch_idx][node_id][0] +=
          stripe_dummy_loads[cur_s][node_id][0];
      batch_node_loads[best_batch_idx][node_id][1] +=
          stripe_dummy_loads[cur_s][node_id][1];
    }
  }
...
void ParallelSolution::prepare(Stripe *stripe, vector<int> fail_node_ids,
                               unordered_map<int, int> &res, string scenario,
                               const vector<vector<int>> &loadTable) {
  ECDAG *ecdag = stripe->getECDAG();
  vector<int> curplacement = stripe->getPlacement();
  ...
  if (scenario == "standby") {
    ...
    for (auto concact_idx : ecdag->_ecConcacts) {
      ...
      res.insert(make_pair(concact_idx, repair_node_id));
      // LOG << "DEBUG concact " << concact_idx << " choose " << repair_node_id
      // << endl; cout << "[INFO] stripe " << stripe->getStripeId() << " fail
      // nodeidx is " << (32767-concact_idx)<< " choose new node " <<
      // repair_node_id <<"standby"<<endl;
    }
  } else {
    // for each stripe
    for (auto concact_idx : ecdag->_ecConcacts) {
      ...
      candidates.clear();
      for (int i = 0; i < _cluster_size; i++) {
        if (find(curplacement.begin(), curplacement.end(), i) ==
            curplacement.end()) {
          if (find(avoid_node_ids.begin(), avoid_node_ids.end(), i) ==
              avoid_node_ids.end())
            candidates.push_back(i);
      ...
      repair_node_id =
          candidates[rand() %
                     candidates.size()]; // scatter场景下 随机选 与yuhan不同！
      avoid_node_ids.push_back(repair_node_id);
      // repair_node_id_concact.push_back(repair_node_id);
      res.insert(make_pair(concact_idx, repair_node_id));
      // LOG << "DEBUG concact " << concact_idx << " choose " << repair_node_id
...
vector<Stripe *>
ParallelSolution::rearrange_stripes(vector<Stripe *> original_list) {
...
void ParallelSolution::genColoringForMultipleFailureLevelNew1(
    Stripe *stripe, vector<int> fail_node_ids, string scenario,
    vector<vector<int>> &loadTable, int method) {
  // 1. 基础准备
  ECDAG *ecdag = stripe->getECDAG();
  ...
  for (int node_id : stripe->_nodelist) {
    ...
    for (int fail_id : fail_node_ids)
      if (node_id == fail_id)
        is_failed = true;
    if (!is_failed)
      unique_cand.insert(node_id);
  }
  // 确保修复目标节点也在候选列表中
  for (int i = 0; i < stripe->getFailNum(); i++) {
    int header_id = ecdag->_ecHeaders[i];
    if (coloring.count(header_id))
      unique_cand.insert(coloring[header_id]);
  }
  ...
  for (int c : candidates)
    cluster_size = max(cluster_size, c + 1);
  for (auto pair : coloring)
    if (pair.second != -1)
      cluster_size = max(cluster_size, pair.second + 1);
...
// 阶段一：初始贪心 (基于拓扑序)
// =========================================================
void ParallelSolution::initialGreedyColoring(ECDAG *ecdag,
                                             unordered_map<int, int> &coloring,
                                             const vector<int> &candidates,
                                             const vector<int> &topoIdxs,
                                             int cluster_size) {
  ...
  for (int u : topoIdxs) {
    ...
    if (coloring.find(u) != coloring.end() && coloring[u] != -1)
      continue;
...
// 阶段二：快速局部搜索 (核心优化逻辑)
// =========================================================
void ParallelSolution::fastLocalSearch(ECDAG *ecdag,
                                       unordered_map<int, int> &coloring,
                                       const vector<int> &candidates,
                                       const vector<int> &itm_idx,
                                       int cluster_size) {
  ...
  for (int iter = 0; iter < max_iter; iter++) {
    ...
    for (int new_machine : shuffled_candidates) {
      ...
      // 为了准确性，我们使用完整增量更新
      if (tryMoveNode(nodeMap, coloring, machine_loads, node_to_move,
                      target_machine, current_max_load)) {
...
bool ParallelSolution::tryMoveNode(const unordered_map<int, ECNode *> &nodeMap,
                                   unordered_map<int, int> &coloring,
                                   vector<int> &machine_loads, int u,
                                   int new_machine, int global_max_load) {
  ...
  if (!creates_new_bottleneck &&
      predicted_old_load < machine_loads[old_machine]) {
    // 确认移动！更新真实数据
...
// 辅助：全量计算负载 (用于初始化和校验)
// =========================================================
void ParallelSolution::recalcGlobalLoad(ECDAG *ecdag,
                                        const unordered_map<int, int> &coloring,
                                        vector<int> &loads) {
