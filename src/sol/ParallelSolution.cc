...
void ParallelSolution::genRepairBatches(int num_failures, vector<int> fail_node_list, int num_agents, string scenario, bool enqueue)
{
    ...
    if (num_failures == 1)
    {
        ...
        genRepairBatchesForMultipleFailureNew(fail_node_list, num_agents, scenario, 0);
...
int ParallelSolution::hungary(int rg_id, int cur_match_stripe, int *matrix, int matrix_start_addr, int *node_selection)
{
    ...
    { // 遍历每个node
        if ((matrix[matrix_start_addr + cur_match_stripe * _num_agents + i] == 1) && (_mark[i] == 0))
        { // 该node是该stripe的存储节点
            ...
            if (node_selection[rg_id * _num_agents + i] == -1)
            { // 该node尚未匹配
                if (_debug)
...
void ParallelSolution::GloballyMLP(Stripe *stripe, const vector<int> &itm_idx, const vector<int> &candidates, ECDAG *ecdag, unordered_map<int, int> &coloring, vector<vector<int>> &loadTable)
{
    if (true)
...
void ParallelSolution::GloballyMLP_yh(Stripe *stripe, const vector<int> &itm_idx, unordered_map<int, int> &coloring, vector<vector<int>> &loadTable)
{
...
void ParallelSolution::improve_hybrid(int fail_node_id, string scenario)
{
    ...
    while (true)
    {
        ...
        if (DEBUG_ENABLE)
            LOG << endl
                << "imporove for batch" << head_batch->getBatchId() << endl;
        int tail_batch_idx = _batch_list.size() - 1;
        int stripe_num = head_batch->getStripeList().size();
        float best_avg = evalTable1(head_batch->getLoadTable(_num_agents + _standby_size))._load * 1.0 / stripe_num;
