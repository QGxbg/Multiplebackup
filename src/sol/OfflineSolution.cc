...
void OfflineSolution::genRepairBatches(int num_failures, vector<int> fail_node_list, int num_agents, string scenario, bool enqueue) {
    ...
    if (num_failures == 1) {
        ...
        genRepairBatchesForMuitipleFailure(fail_node_list, num_agents, scenario, enqueue);
    }

    _finish_gen_batches = true;
}

void OfflineSolution::genRepairBatchesForSingleFailure(int fail_node_id, int num_agents, string scenario, bool enqueue) {
    ...
    for (int batchid=0; batchid<_num_batches; batchid++) {
        ...
        for (int i=0; i<_batch_size; i++) {
            ...
            if (stripeidx < _stripes_to_repair.size()) {
                ...
                genOfflineColoringForSingleFailure(curstripe, curcoloring, fail_node_id, num_agents, scenario);
...
void OfflineSolution::genOfflineColoringForSingleFailure(Stripe* stripe, unordered_map<int, int>& res, 
    ...
    //cout << "OfflineSolution::genOfflineColoringForSingleFailure start" << endl;
    //LOG << "fail_node_id:"<<fail_node_id<<endl;
...
void OfflineSolution::genRepairBatchesForMuitipleFailure(vector<int> fail_node_id, int num_agents, string scenario, bool enqueue) {
    ...
    for (int batchid=0; batchid<_num_batches; batchid++) {
        ...
        for (int i=0; i<_batch_size; i++) {
            ...
            if (stripeidx < _stripes_to_repair.size()) {
                ...
                vector<int> tmp_fnid;
                //tmp_fnid.push_back(fail_node_id);
                tmp_fnid.insert(tmp_fnid.end(),fail_node_id.begin(),fail_node_id.end());
                ECDAG* curecdag = curstripe->genRepairECDAG(_ec, tmp_fnid);
                //ECDAG* curecdag = curstripe->genRepairECDAG(_ec, fail_node_id);

                // 1.2 generate offline coloring for the current stripe
                unordered_map<int, int> curcoloring;
                genOfflineColoringForMultipleFailure(curstripe, curcoloring, fail_node_id, num_agents, scenario);
...
void OfflineSolution::genOfflineColoringForMultipleFailure(Stripe* stripe, unordered_map<int, int>& res, 
    ...
    //cout << "OfflineSolution::genOfflineColoringForMultipleFailure start" << endl;
    //LOG << "fail_node_id:"<<fail_node_id<<endl;
    // map a sub-packet idx to a real physical node id
