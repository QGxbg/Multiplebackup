...
void ECDAG::removeChild(ECNode* pnode){
...
unordered_map<int, ECNode*>& ECDAG::getECNodeMapNew() {
  return _ecNodeMap;
}
...
// FIXME: 之前的gentopo逻辑中,会跳过idx=REQUESTOR的节点,所以在改动REQUESTOR的逻辑之后,会有问题
// 所以应该让函数知道有哪些concact节点
vector<int> ECDAG::genTopoIdxs() {
  ...
  while (leaves.size() > 0) {
    ...
    if(find(_ecConcacts.begin(), _ecConcacts.end(), idx) == _ecConcacts.end() && find(_ecLeaves.begin(), _ecLeaves.end(), idx) == _ecLeaves.end()){
        topoIdxs.push_back(idx);
    }
    ECNode* node = _ecNodeMap[idx];
    if (outref[idx] == 0) {
