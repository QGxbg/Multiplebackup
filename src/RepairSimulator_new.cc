...
//
// 用法: ./RepairSimulator_new code ecn eck ecw num_agents trace_file method
// scenario placement_file
...
void applyRepairCost(const vector<int> &repaired_nodes,
                     const vector<Stripe *> &stripe_list, string code, int ecn,
                     int eck, int ecw, string scenario, int method,
                     Config *conf, ECBase *ec, RepairSchedulerNew &scheduler,
                     bool verbose = false) {
  if (repaired_nodes.empty())
    return;
  ...
  if (verbose) {
    ...
    for (int nid : repaired_nodes)
      cout << "Node-" << nid << " ";
    cout << "\n  [Repair] affected stripes: " << affected_stripes.size()
         << " stripes:" << endl;
  ...
  if (verbose) {
    printf("  [Cost] batches=%d load=%.2f blks bdwt=%.2f blks\n",
           (int)batches.size(), 1.0 * total_load / ecw, 1.0 * total_bdwt / ecw);
  }

  scheduler.recordRepairLoad(1.0 * total_load / ecw, 1.0 * total_bdwt / ecw);
  delete sol;
}
...
int checkStripeDataLoss(const vector<int> &current_failed_nodes,
                        const vector<Stripe *> &stripe_list, int ecn, int eck) {
...
// 通用的数据丢失应急处理
// =================================================================================
void handleDataLoss(RepairSchedulerNew &scheduler, bool verbose,
                    int loss_stripes) {
  // 只记录丢失事件和条带数，不修复，坏节点继续留在 _current_failed
  // 后续这些节点涉及的其他条带若变危险，再触发正常修复流程
  scheduler.recordDataLoss(loss_stripes);
  if (verbose) {
    printf("  *** DATA LOSS! %d stripes lost (failed nodes remain, will repair "
           "when other stripes at risk) ***\n",
           loss_stripes);
  }
}
...
// 原有三种策略（适配新调度器接口）
// =================================================================================
RepairStats runOriginalStrategy(int strategy, TraceReader &reader, string code,
  ...
  while (reader.hasCurrentEvent()) {
    ...
    if (verbose) {
      cout << "[Day " << event->timestamp
           << "] new_fails=" << event->fail_node_ids.size()
    ...
    if (strategy == 0) {
      ...
      if (decision == REPAIR_IMMEDIATE) {
        ...
        if (verbose)
          printf("  -> REPAIR %d nodes  [Reason: Immediate]\n",
                 (int)failed.size());
        applyRepairCost(failed, stripe_list, code, ecn, eck, ecw, scenario,
                        method, conf, ec, scheduler);
      }
    } else if (strategy == 1) {
      ...
      if (decision == REPAIR_IMMEDIATE) {
        ...
        if (verbose)
          printf("  -> REPAIR %d nodes  [Reason: Lazy stripe-level, some "
                 "stripe cur >= tolerance=%d]\n",
                 (int)repaired.size(), ecn - eck);
        applyRepairCost(repaired, stripe_list, code, ecn, eck, ecw, scenario,
                        method, conf, ec, scheduler);
      } else {
        if (verbose)
      ...
      if (decision == REPAIR_IMMEDIATE || decision == REPAIR_PARTIAL) {
        ...
        if (verbose)
          printf("  -> SKIP  [No dangerous stripe: all stripes safe with "
                 "cur+next <= tolerance=%d]\n",
                 ecn - eck);
      }
    }
  ...
  if (scheduler.getCurrentFailureCount() > 0) {
    vector<int> failed = scheduler.executeFullRepair();
    applyRepairCost(failed, stripe_list, code, ecn, eck, ecw, scenario, method,
                    conf, ec, scheduler);
  }
...
// =================================================================================
RepairStats runRiskAwarePartial(TraceReader &reader, string code, int ecn,
                                int eck, int ecw,
                                const vector<Stripe *> &stripe_list, ECBase *ec,
                                int num_agents, string scenario, int method,
                                Config *conf, bool verbose,
  ...
  while (reader.hasCurrentEvent()) {
    TraceEvent *event = reader.getCurrentEvent();
    scheduler.addFailures(event->fail_node_ids);
    int loss_stripes = checkStripeDataLoss(scheduler.getCurrentFailedNodes(),
                                           stripe_list, ecn, eck);
    ...
    if (decision == REPAIR_IMMEDIATE) {
      vector<int> failed = scheduler.executeFullRepair();
      if (verbose)
        cout << "  -> FULL REPAIR " << failed.size() << " nodes" << endl;
      applyRepairCost(failed, stripe_list, code, ecn, eck, ecw, scenario,
                      method, conf, ec, scheduler);
      ...
      if (verbose)
        cout << "  -> PARTIAL REPAIR " << repaired.size() << "/" << cur.size()
             << " nodes" << endl;
      applyRepairCost(repaired, stripe_list, code, ecn, eck, ecw, scenario,
                      method, conf, ec, scheduler);
    } else {
...
// =================================================================================
RepairStats runStripeDifferentiated(TraceReader &reader, string code, int ecn,
                                    int eck, int ecw,
                                    const vector<Stripe *> &stripe_list,
                                    ECBase *ec, int num_agents, string scenario,
                                    int method, Config *conf, bool verbose,
  ...
  while (reader.hasCurrentEvent()) {
    ...
    vector<int> next_ids =
        next_event ? next_event->fail_node_ids : vector<int>{};
    ...
    if (decision == REPAIR_IMMEDIATE || decision == REPAIR_PARTIAL) {
      ...
      if (decision == REPAIR_IMMEDIATE) {
        ...
        if (verbose)
          cout << "  -> FULL REPAIR " << repaired.size() << " nodes" << endl;
      } else {
        repaired = scheduler.executePartialRepair(nodes_to_repair);
        if (verbose)
          cout << "  -> STRIPE-DRIVEN PARTIAL REPAIR " << repaired.size() << "/"
      ...
      if (verbose)
        cout << "  -> SKIP (no critical stripes)" << endl;
    }
...
// Strategy 5: Combined
// =================================================================================
RepairStats runCombined(TraceReader &reader, string code, int ecn, int eck,
                        int ecw, const vector<Stripe *> &stripe_list,
                        ECBase *ec, int num_agents, string scenario, int method,
                        Config *conf, bool verbose,
                        double threshold_partial = 0.5,
                        double threshold_full = 0.8,
  ...
  if (verbose) {
    ...
    cout << "  risk_partial=" << threshold_partial
         << " risk_full=" << threshold_full << " stripe_thr=" << stripe_risk_thr
         << endl;
    cout << "======================================================" << endl;
  }
  ...
  while (reader.hasCurrentEvent()) {
    ...
    auto result_cb =
        scheduler.decideCombined(stripe_list, future_windows, threshold_partial,
                                 threshold_full, stripe_risk_thr);
    ...
    if (decision == REPAIR_IMMEDIATE) {
      ...
      vector<int> repaired = scheduler.executePartialRepair(nodes_to_repair);
      if (verbose)
        cout << "  -> COMBINED PARTIAL REPAIR " << repaired.size() << " nodes"
             << " (remaining: " << scheduler.getCurrentFailureCount() << ")"
             << endl;
      applyRepairCost(repaired, stripe_list, code, ecn, eck, ecw, scenario,
                      method, conf, ec, scheduler);
    } else {
...
// =================================================================================
RepairStats runMLDriven(TraceReader &reader, string code, int ecn, int eck,
                        int ecw, const vector<Stripe *> &stripe_list,
                        ECBase *ec, int num_agents, string scenario, int method,
                        Config *conf, bool verbose, double fpr, double fnr,
                        int seed = 42) {
  RepairSchedulerNew scheduler(ecn, eck, ecw);
  reader.reset();
  ...
  while (reader.hasCurrentEvent()) {
    ...
    if (verbose) {
      // 打印预测 vs 真实对比
      vector<int> real_next =
          next_real ? next_real->fail_node_ids : vector<int>{};
      ...
      for (int nid : real_next)
        if (!pred_set.count(nid))
          fn.push_back(nid);
      printf("+-- [Day %d] ML Prediction: real_next=%d pred=%d  TP=%d FP=%d "
             "FN=%d\n",
    ...
    if (decision == REPAIR_IMMEDIATE || decision == REPAIR_PARTIAL) {
      ...
      applyRepairCost(repaired, stripe_list, code, ecn, eck, ecw, scenario,
                      method, conf, ec, scheduler, verbose);
    } else {
      if (verbose)
        printf(
            "  -> SKIP  [No dangerous stripe with pred_ids, safe to wait]\n");
    }
...
// 打印详细对比表
// =================================================================================
void printComparisonTable(const string names[], const RepairStats stats[],
                          int n, int event_count, const string &code, int ecn,
                          int eck) {
  cout << "\n============================================================"
       << endl;
  ...
  printf("%-22s | %6s | %10s | %10s | %8s | %10s | %7s | %5s\n", "Strategy",
         "Rounds", "Load(blks)", "Bdwt(blks)", "LossEvents", "LostStripes",
         "MaxFail", "Skips");
  printf("%-22s-|-%6s-|-%10s-|-%10s-|-%8s-|-%10s-|-%7s-|-%5s\n",
         "----------------------", "------", "----------", "----------",
         "----------", "-----------", "-------", "-----");
  ...
  for (int i = 0; i < n; i++) {
    printf("%-22s | %6d | %10.2f | %10.2f | %8d | %10d | %7d | %5d\n",
           names[i].c_str(), stats[i].total_repair_rounds,
           stats[i].total_repair_load, stats[i].total_repair_bdwt,
           stats[i].data_loss_events, stats[i].lost_stripes_count,
           stats[i].max_concurrent_failures, stats[i].skipped_repair_count);
  }
  cout << "============================================================"
       << endl;
  ...
  if (stats[0].total_repair_load > 0) {
    ...
    for (int i = 1; i < n; i++) {
      ...
      printf("  %-22s: %+.1f%% (data_loss=%d, partial_repairs=%d)\n",
             names[i].c_str(), save, stats[i].data_loss_events,
...
int main(int argc, char **argv) {
  if (argc < 10) {
    ...
    cout << "  Optional:" << endl;
    cout << "   10. threshold_partial  (default 0.5,  unused by ML strategies)"
         << endl;
    cout << "   11. threshold_full     (default 0.8,  unused by ML strategies)"
         << endl;
    cout << "   12. stripe_risk_thr    (default 0.75, unused by ML strategies)"
         << endl;
    cout << "   13. ml_fpr_low         (default 0.002, ML-Low false positive "
            "rate)"
         << endl;
    cout << "   14. ml_fpr_high        (default 0.025, ML-High false positive "
            "rate)"
         << endl;
    cout << "   15. ml_fnr             (default 0.05,  both ML strategies "
            "false negative rate)"
         << endl;
    cout << "   16. run_mode            (default 0)" << endl;
    cout << "         0 = all 5 strategies" << endl;
    cout << "         1 = baseline only  (Immediate / Lazy / Perfect)" << endl;
    cout << "         2 = ML only        (ML-Low / ML-High, skip baseline)"
         << endl;
    cout << endl;
    cout << "Examples:" << endl;
    cout << "  # Run all strategies (default):" << endl;
    cout << "  ./RepairSimulator_new Clay 14 10 4 40 trace.txt 1 standby "
            "placement.txt"
         << endl;
    cout << "  # Run baseline only once:" << endl;
    cout << "  ./RepairSimulator_new Clay 14 10 4 40 trace.txt 1 standby "
            "placement.txt 0.5 0.8 0.75 0.002 0.025 0.05 1"
         << endl;
    cout << "  # FNR scan - ML only:" << endl;
    cout << "  ./RepairSimulator_new Clay 14 10 4 40 trace.txt 1 standby "
            "placement.txt 0.5 0.8 0.75 0.002 0.025 <fnr> 2"
         << endl;
    cout << "  # FPR scan - ML only:" << endl;
    cout << "  ./RepairSimulator_new Clay 14 10 4 40 trace.txt 1 standby "
            "placement.txt 0.5 0.8 0.75 <fpr> <fpr> 0.05 2"
         << endl;
    return 0;
  }
  ...
  // 可选参数（argv[10]~[15]）：
  //   [10] threshold_partial  风险分阈值（部分修复）  默认 0.5
  //   （仅旧策略使用，ML不用） [11] threshold_full     风险分阈值（全量修复）
  //   默认 0.8  （仅旧策略使用，ML不用） [12] stripe_risk_thr    条带风险阈值
  //   默认 0.75 （仅旧策略使用，ML不用） [13] ml_fpr_low         ML-Low 误报率
  //   默认 0.002 (0.2%) [14] ml_fpr_high        ML-High 误报率          默认
  //   0.025 (2.5%) [15] ml_fnr             两种ML策略共用漏报率    默认 0.05
  //   (5%)
  //
  // 实验用法示例：
  //   FNR敏感性扫描（固定FPR，改变FNR）：
  //     ./RepairSimulator_new Clay 14 10 4 40 trace.txt 1 standby placement.txt
  //     //         0.5 0.8 0.75 0.002 0.025 <fnr>
  //
  //   FPR敏感性扫描（固定FNR，改变FPR）：
  //     ./RepairSimulator_new Clay 14 10 4 40 trace.txt 1 standby placement.txt
  //     //         0.5 0.8 0.75 <fpr_low> <fpr_high> 0.05
  ...
  cout << "============================================" << endl;
  cout << "Code: " << code << " (" << ecn << "," << eck << ") ecw=" << ecw
       << endl;
  cout << "Nodes: " << num_agents << " | Fault tolerance: " << (ecn - eck)
       << endl;
  ...
  if (run_mode == 0 || run_mode == 1) {
    ...
    stats[0] =
        runOriginalStrategy(0, reader, code, ecn, eck, ecw, stripe_list, ec,
                            num_agents, scenario, method, conf, verbose);

    cout << "  [2/3] Lazy..." << endl;
    stats[1] =
        runOriginalStrategy(1, reader, code, ecn, eck, ecw, stripe_list, ec,
                            num_agents, scenario, method, conf, verbose);

    cout << "  [3/3] Perfect Prediction (TraceDriven)..." << endl;
    stats[2] =
        runOriginalStrategy(2, reader, code, ecn, eck, ecw, stripe_list, ec,
                            num_agents, scenario, method, conf, verbose);
  }
  ...
  if (run_mode == 0 || run_mode == 2) {
    ...
    stats[3] =
        runMLDriven(reader, code, ecn, eck, ecw, stripe_list, ec, num_agents,
                    scenario, method, conf, verbose, ml_fpr_low, ml_fnr);

    printf("  [ML-High] fpr=%.4f fnr=%.4f ...\n", ml_fpr_high, ml_fnr);
    stats[4] =
        runMLDriven(reader, code, ecn, eck, ecw, stripe_list, ec, num_agents,
                    scenario, method, conf, verbose, ml_fpr_high, ml_fnr, 123);
  }
  ...
  if (run_mode == 0) {
    // 全部策略输出
    printComparisonTable(names, stats, NUM_STRATEGIES, reader.getEventCount(),
                         code, ecn, eck);
  } else if (run_mode == 1) {
    // 只输出基准三种
    printComparisonTable(names, stats, 3, reader.getEventCount(), code, ecn,
                         eck);
  } else {
    ...
    printf("%-22s | %10s | %10s | %8s | %10s\n", "Strategy", "Load(blks)",
           "Bdwt(blks)", "LossEvents", "LostStripes");
  ...
  if (run_mode != 2) {
    ...
    printf("%-22s | %10s | %8s | %8s | %8s | %s\n", "Strategy", "LoadSaving%",
           "DataLoss", "Partial", "ActualFPR", "Note");
    int pareto_n = (run_mode == 1) ? 3 : NUM_STRATEGIES;
    for (int i = 0; i < pareto_n; i++) {
      double save = (stats[0].total_repair_load > 0)
                        ? (1.0 - stats[i].total_repair_load /
                                     stats[0].total_repair_load) *
                              100
                        : 0.0;
      string note = "";
      if (stats[i].data_loss_events == 0)
        note = "Safe";
      else if (stats[i].data_loss_events <= 2)
        note = "Low risk";
      else
        note = "Risky";
      printf("%-22s | %10.1f | %8d | %8d | %8.4f | %s\n", names[i].c_str(),
             save, stats[i].data_loss_events, stats[i].partial_repair_count,
