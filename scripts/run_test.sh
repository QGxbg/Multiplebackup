...
echo ">>> Step 2: Generating data blocks (data/gen_standalone_data_multiple.py) ..."
...
echo ">>> Step 3: Starting cluster nodes (start.py) ..."
...
METHOD="offline"                # centralize, offline, parallel
SCENARIO="scatter"              # standby, scatter
FAIL_NODE_NUM=$(echo $FAIL_IDS | wc -w)

echo ">>> Step 4: Running MultipleCoor ..."
cd ${PROJ_DIR}
